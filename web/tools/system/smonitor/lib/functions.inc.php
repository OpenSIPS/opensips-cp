...
function get_mi_modules($mi_url)
{
	...
	$message=mi_command("statistics:get", array("statistics"=>array("all")), $mi_url, $errors);
	if (!empty($errors))
		return;
...
function get_vars($module, $mi_url)
{
	...
	$message=mi_command( "statistics:get", array("statistics"=>array($module.":")), $mi_url, $errors);
	if (!empty($errors))
		return;
...
function get_vars_type( $mi_url )
{
	...
	$message=mi_command("statistics:list", NULL, $mi_url, $errors);
	if (!empty($errors))
		return;
...
function get_all_vars( $mi_url , $stats_list)
{
	...
	$message=mi_command( "statistics:get", array("statistics"=>$list), $mi_url, $errors);
	if (!empty($errors)) 
		return;
...
function get_stats_list($box_id) {
	...
	require(__DIR__."/db_connect.php");
	$sql = "SELECT name, min(time) as time FROM ocp_monitoring_stats WHERE name = ? AND box_id = ? group by name order by time asc";
	$stm = $link->prepare($sql);
