...
function get_mi_identifiers($mi_url)
{
	global $config;
	...
	if (!empty($errors))
		return;
	$temp = array();
	foreach ($message as $group){
		foreach($group['Identifiers'] as $gr_identifier)
			$temp [] = $group['Group'].": ".$gr_identifier;
	}
	$message = implode("\n",$temp);

	$identifiers=$temp;

        $_SESSION['identifiers_no']=count($identifiers) ;
		$i = 0;
        foreach ($identifiers as $identifier)
        {   
			$_SESSION['identifier_name'][$i] = $identifier;
			if (!isset($_SESSION['identifier_open'][$i]))
