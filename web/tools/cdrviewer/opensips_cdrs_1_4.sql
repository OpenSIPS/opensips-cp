...
BEGIN
  ...
  DECLARE inv_cursor CURSOR FOR SELECT time, callid, from_tag, to_tag FROM opensips_1_4.acc where method='INVITE' and cdr_id='0';
  ...
  REPEAT
    ...
    IF NOT done THEN
      ...
      IF bye_record = 1 THEN
        INSERT INTO cdrs (call_start_time,duration,sip_call_id,sip_from_tag,sip_to_tag,created) VALUES (v_inv_time,UNIX_TIMESTAMP(v_bye_time)-UNIX_TIMESTAMP(v_inv_time),v_callid,v_from_tag,v_to_tag,NOW());
